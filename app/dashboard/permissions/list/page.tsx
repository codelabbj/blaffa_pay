...
export default function PermissionsListPage() {
  ...
  const [userPlatformsModalOpen, setUserPlatformsModalOpen] = useState(false)
  ...
  const handleOpenUserPlatforms = async (permission: Permission) => {
    setUserPlatformsModalOpen(true)
    setUserPlatformsLoading(true)
    setUserPlatformsError("")
    setUserPlatformsData(null)
    try {
      const endpoint = `${baseUrl.replace(/\/$/, "")}/api/payments/betting/admin/permissions/user_platforms/?user_uid=${permission.partner}`
      const data = await apiFetch(endpoint)
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-orange-50 via-gray-50 to-green-50 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {/* User Platforms Modal */}
        <Dialog open={userPlatformsModalOpen} onOpenChange={setUserPlatformsModalOpen}>
          <DialogContent className="max-w-6xl max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              <DialogTitle className="flex items-center space-x-2">
                <Users className="h-5 w-5 text-purple-600" />
                <span>Permissions Plateformes Utilisateur</span>
              </DialogTitle>
            </DialogHeader>
